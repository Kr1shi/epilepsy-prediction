...
class EEGPreprocessor:
    def __init__(self, fold_config: Dict):
        ...
        self.logger.info("EEG Preprocessor initialized (Per-Sequence Zero-Centering Mode)")
    ...
    def preprocess_sequences_from_file(self, patient_id: str, filename: str, sequences: List[Dict]) -> List[Optional[Dict]]:
        ...
        try:
            ...
            all_segment_starts = [start for seq in sequences for start in seq['segment_starts']]
            min_time, max_time = min(all_segment_starts), max(all_segment_starts) + SEGMENT_DURATION
            padding = 5.0
            crop_tmin, crop_tmax = max(0, min_time - padding), min(raw_selected.times[-1], max_time + padding)
            raw_cropped = raw_selected.copy().crop(tmin=crop_tmin, tmax=crop_tmax)

            raw_cropped.filter(l_freq=LOW_FREQ_HZ, h_freq=HIGH_FREQ_HZ, fir_design='firwin', n_jobs=MNE_N_JOBS, verbose=False)
            raw_cropped.notch_filter(freqs=NOTCH_FREQ_HZ, n_jobs=MNE_N_JOBS, verbose=False)
    ...
    def append_to_hdf5(self, split_name: str, batch_sequences: List[Dict]):
        ...
        with h5py.File(output_file, 'a') as f:
            if 'spectrograms' not in f:
                f.create_dataset('spectrograms', (0, n_seq_len, n_channels, n_freqs, n_times),
                                 maxshape=(None, n_seq_len, n_channels, n_freqs, n_times), dtype=np.float32, chunks=True)
                ...
                for k, dt in [('start_times', np.float32), ('end_times', np.float32), ('file_names', 'S20'), 
                             ('time_to_seizure', np.float32), ('segment_ids', 'S50')]:
                    seg_info.create_dataset(k, (0,), maxshape=(None,), dtype=dt, chunks=True)
                f.create_group('metadata').attrs['normalization'] = 'per-sequence-zero-centering'

            n_existing = f['spectrograms'].shape[0]
            n_new = len(batch_sequences)
            new_total = n_existing + n_new
            for ds in [f['spectrograms'], f['labels'], f['patient_ids']] + [f['segment_info'][k] for k in f['segment_info']]:
            ...
            for i, seq in enumerate(batch_sequences):
                ...
                si['segment_ids'][idx] = f"{seq['patient_id']}_{seq['sequence_start_sec']}".encode('utf-8')
    ...
    def run_preprocessing(self):
        ...
        splits = {s: [seq for seq in all_seqs if seq.get('split') == s] for s in ['train', 'test']}
        for split_name in ['train', 'test']:
            self.process_and_save_split(split_name, splits[split_name])
        
        self.logger.info(f"Preprocessing complete in {(time.time()-start_time)/60:.1f}m")
