...
class EEGPreprocessor:
    def __init__(self, input_json_path: str = None, fold_config: Dict = None):
        ...
        self.logger.info(
            f"Multiprocessing: Using {self.n_workers}/{available_cores} CPU cores for parallel preprocessing"
        )
        ...
        if self.checkpoint_file.exists():
            try:
                ...
                if isinstance(checkpoint.get("processed_segments"), list):
                    checkpoint["processed_segments"] = set(
                        checkpoint["processed_segments"]
                    )

                self.logger.info(
                    f"Loaded checkpoint: {len(checkpoint.get('processed_segments', []))} segments processed"
                )
                return checkpoint
    ...
    def save_checkpoint(self):
        ...
        if isinstance(checkpoint_copy.get("processed_segments"), set):
            checkpoint_copy["processed_segments"] = list(
                checkpoint_copy["processed_segments"]
            )

        # Handle validation stats sets
        if (
            "valid_segments" in checkpoint_copy
            and "stats" in checkpoint_copy["valid_segments"]
        ):
            stats = checkpoint_copy["valid_segments"]["stats"]
            if "patients_processed" in stats and isinstance(
                stats["patients_processed"], set
            ):
                stats["patients_processed"] = list(stats["patients_processed"])
            if "patients_with_no_valid" in stats and isinstance(
                stats["patients_with_no_valid"], set
            ):
            ...
            for sequence in sequences:
                try:
                    ...
                    for segment_start in sequence["segment_starts"]:
                        ...
                        if (
                            apply_normalization
                            and self.global_mean is not None
                            and self.global_std is not None
                        ):
                            power_spectrogram = (
                                power_spectrogram - self.global_mean
                    ...
                    self.logger.error(
                        f"Failed to process sequence from {patient_id}/{filename} at {sequence['sequence_start_sec']}s: {e}"
                    )
    ...
    def append_to_hdf5(self, split_name: str, batch_sequences: List[Dict]):
        ...
        self.logger.debug(
            f"Appending {len(batch_sequences)} sequences to {output_file}"
        )
        ...
        try:
            with h5py.File(output_file, "a") as f:
                ...
                if "spectrograms" not in f:
                    self.logger.info(
                        f"Creating new HDF5 file or datasets in: {output_file}"
                    )
                    ...
                    seg_info.create_dataset(
                        "file_names", (0,), maxshape=(None,), dtype="S20", chunks=True
                    )
                    seg_info.create_dataset(
                        "time_to_seizure",
                        (0,),
                    ...
                    seg_info.create_dataset(
                        "segment_ids", (0,), maxshape=(None,), dtype="S50", chunks=True
                    )
                    ...
                    metadata.attrs["target_channels"] = [
                        ch.encode("utf-8") for ch in batch_sequences[0]["channel_names"]
                    ]
                    ...
                    metadata.attrs["stft_params"] = (
                        f"nperseg={STFT_NPERSEG}, noverlap={STFT_NOVERLAP}"
                    )
                ...
                f["metadata"].attrs[
                    "class_distribution"
                ] = f"{self.positive_label}: {n_positive}, interictal: {n_interictal}"

            self.logger.debug(
                f"Successfully appended batch. New total sequences for {split_name}: {new_total}"
            )
        except Exception as e:
    ...
    def run_preprocessing(self):
        ...
        try:
            ...
            if "validation_info" in data:
                ...
                self.logger.info(
                    f"Channel validation (from segmentation): {val_info.get('files_with_valid_channels', 'N/A')}/{val_info.get('total_files_checked', 'N/A')} files valid"
                )
                if val_info.get("files_with_invalid_channels", 0) > 0:
                    self.logger.info(
                        f"Files skipped due to missing channels: {val_info['files_with_invalid_channels']}"
                    )
            ...
            self.logger.info(
                "Using all sequences (channel validation performed during segmentation)"
            )

            # Determine data splits
            if not self.checkpoint.get("splits_created", False):
                if valid_sequences and all(
                    sequence.get("split") for sequence in valid_sequences
                ):
                    self.logger.info(
                        "Detected pre-assigned splits in segmentation metadata; using them directly."
                    )
                    ...
                    for sequence in valid_sequences:
                        ...
                        if split_name in splits:
                            ...
                            ignored_sequences[split_name] = (
                                ignored_sequences.get(split_name, 0) + 1
                            )
                    ...
                    for split_name, split_sequences in splits.items():
                        positive_count = sum(
                            ...
                            if s["type"] == self.positive_label
                        )
                        interictal_count = len(split_sequences) - positive_count
                        self.logger.info(
                            f"{split_name}: {len(split_sequences)} sequences "
                            f"({positive_count} {self.positive_label}, {interictal_count} interictal)"
                        )

                    if any(
                        split_sequences == [] for split_sequences in splits.values()
                    ):
                        empty_splits = [
                            name for name, seqs in splits.items() if not seqs
                        ]
                        raise ValueError(
                            f"Empty splits detected in segmentation metadata: {empty_splits}"
                        )
            ...
            if not self.checkpoint.get("normalization_stats_computed", False):
                self.logger.info("=" * 60)
                self.logger.info(
                    "COMPUTING NORMALIZATION STATISTICS AND PROCESSING TRAINING DATA"
                )
                self.logger.info("=" * 60)
                ...
                if not train_sequences:
                    raise ValueError(
                        "Training split is empty; cannot compute normalization statistics."
                    )

                self.logger.info(
                    f"Processing {len(train_sequences)} training samples to compute normalization stats..."
                )
                ...
                self.logger.info(
                    f"Processing {len(train_sequences)} sequences from {total_files} unique files"
                )

                with tqdm(
                    total=len(train_sequences), desc="Processing training data"
                ) as pbar:
                    for (patient_id, filename), file_sequences in file_groups.items():
                        ...
                        for sequence, processed in zip(
                            file_sequences, processed_file_sequences
                        ):
                            if processed:
                                ...
                                self.checkpoint["processed_count"] = (
                                    self.checkpoint.get("processed_count", 0) + 1
                                )
                            pbar.update(1)

                if not train_spectrograms:
                    raise ValueError(
                        "Failed to process any training sequences for normalization stats!"
                    )
                ...
                self.logger.info(
                    f"Applying normalization and saving {len(train_processed_data)} training sequences..."
                )
                batch_sequences = []
                for processed in tqdm(
                    train_processed_data, desc="Normalizing and saving training data"
                ):
                    # Apply normalization in-place
                    processed["spectrogram"] = (
                        processed["spectrogram"] - self.global_mean
                    ) / (self.global_std + 1e-8)
                ...
                self.logger.info(
                    "Training data processing completed during stats computation phase"
                )

            # Load normalization statistics before processing
            if not self.load_normalization_stats():
                raise ValueError(
                    "Failed to load normalization statistics! Cannot proceed."
                )
            ...
            for split_name in split_names:
                ...
                if output_file.exists():
                    try:
                        with h5py.File(output_file, "r") as f:
                            if (
                                "segment_info" in f
                                and "segment_ids" in f["segment_info"]
                            ):
                                existing_ids = f["segment_info"]["segment_ids"][:]
                                existing_sequence_ids = {
                                    sid.decode("utf-8") for sid in existing_ids
                                }
                        self.logger.info(
                            f"Found existing {split_name} file with {len(existing_sequence_ids)} sequences"
                        )
                    except Exception as e:
                        self.logger.warning(
                            f"Could not read existing {split_name} file: {e}"
                        )
                ...
                for sequence in split_sequences:
                    sequence_id = (
                        f"{sequence['patient_id']}_{sequence['sequence_start_sec']}"
                    )
                    if sequence_id not in existing_sequence_ids:
                        sequences_to_process.append(sequence)

                self.logger.info(
                    f"Need to process {len(sequences_to_process)}/{len(split_sequences)} sequences for {split_name}"
                )
                ...
                if sequences_to_process:
                    ...
                    self.logger.info(
                        f"Processing {len(sequences_to_process)} sequences from {total_files} unique files"
                    )
                    self.logger.info(
                        f"Average {len(sequences_to_process)/total_files:.1f} sequences per file"
                    )

                    with tqdm(
                        total=len(sequences_to_process), desc=f"Processing {split_name}"
                    ) as pbar:
                        # Iterate over file groups (MAJOR OPTIMIZATION: read/filter each file only ONCE)
                        for (
                            patient_id,
                            filename,
                        ), file_sequences in file_groups.items():
                            # Process ALL sequences from this file in one pass
                            processed_file_sequences = (
                                self.preprocess_sequences_from_file(
                                    patient_id, filename, file_sequences
                                )
                            )

                            # Add successfully processed sequences to batch
                            for sequence, processed_sequence in zip(
                                file_sequences, processed_file_sequences
                            ):
                                sequence_id = f"{sequence['patient_id']}_{sequence['sequence_start_sec']}"
                                ...
                                if processed_sequence:
                                    ...
                                    self.checkpoint["processed_segments"].add(
                                        sequence_id
                                    )
                            ...
                            if len(batch_sequences) >= 10:
                                ...
                                rate = (
                                    self.checkpoint["processed_count"] / elapsed
                                    if elapsed > 0
                                    else 0
                                )
                                remaining_total = (
                                    self.checkpoint["total_sequences"]
                                    - self.checkpoint["processed_count"]
                                )
                                eta = remaining_total / rate if rate > 0 else 0
                                self.logger.info(
                                    f"Progress: {self.checkpoint['processed_count']}/{self.checkpoint['total_sequences']} "
                ...
                if not sequences_to_process and not existing_sequence_ids:
                    self.logger.warning(
                        f"No sequences processed for {split_name} and no existing file found"
                    )
                ...
                if total_removed > 0:
                    self.logger.info(
                        f"{split_name} split - Removed segments: {total_removed} total "
            ...
            self.logger.info(
                f"Processed segments: {self.checkpoint['processed_count']}"
            )
            self.logger.info(
                f"Average rate: {self.checkpoint['processed_count']/total_time:.1f} segments/second"
            )
            self.logger.info("=" * 60)
