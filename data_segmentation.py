...
def create_sequences_all_patients():
    ...
    for i in range(1, 25):
        ...
        if (
            i == 24
        ):  # Skip patient 24 (no interictal sequences due to missing file times)
            continue
...
def assign_single_patient_splits(
    sequences, seizure_splits, interictal_ratios, fold_id, random_seed=None
):
    ...
    seizure_splits = {"train": train_seizures, "test": [test_seizure]}
    print(
        f"LOOCV Fold {fold_id}: Test seizure={test_seizure}, Train seizures={train_seizures}"
    )
    ...
    for sequence in sequences:
        if sequence["type"] == positive_label:
            ...
            assigned_split = next(
                (
                    split_name
                    for split_name, seizure_ids in seizure_splits.items()
                    if seizure_id in seizure_ids
                ),
                None,
            )
    ...
    if interictal_pool:
        ...
        ratio_sum = sum(
            max(interictal_ratios.get(split_name, 0.0), 0.0)
            for split_name in required_splits
        )
        if ratio_sum <= 0:
            normalized = {
                split_name: 1.0 / len(required_splits) for split_name in required_splits
            }
        else:
            normalized = {
    ...
    split_counts = {
        split_name: {
            ...
            positive_label: sum(
                1 for seq in split_sequences if seq["type"] == positive_label
            ),
            "interictal": sum(
                1 for seq in split_sequences if seq["type"] == "interictal"
            ),
        }
