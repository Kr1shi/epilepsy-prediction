...
def create_sequences_from_file(patient_id, filename, all_seizures_global, file_duration, file_offset):
    ...
    sequences = []
    skipped_boundary_sequences = 0

    # 1. Define these parameters at the start so they are available everywhere
    sequence_duration = SEGMENT_DURATION * SEQUENCE_LENGTH 
    stride_duration = SEGMENT_DURATION * SEQUENCE_STRIDE   
    ...
    while sequence_start_local + sequence_duration + SAFETY_MARGIN <= file_duration:
        ...
        # Get global time of the LAST segment in the sequence
        ...
        p_start_sec = PREICTAL1_START_MIN * 60
        p_end_sec = PREICTAL1_END_MIN * 60
        i_start_sec = PREICTAL2_START_MIN * 60
        i_end_sec = PREICTAL2_END_MIN * 60

        if TASK_MODE == 'prediction':
            for seizure in all_seizures_global:
                seizure_start = seizure['start_sec_global']
                dt = seizure_start - last_segment_end_global

                # CHECK 1: TRUE CLASS (Preictal) [X min to Y min]
                if p_end_sec <= dt <= p_start_sec:
                    sequence_type = 'preictal'
                    time_to_seizure = dt
                    matched_seizure_id = seizure.get('seizure_id')
                    break 
            
            # CHECK 2: NEGATIVE CLASS (Interictal) [Z min to J min]
            if sequence_type is None:
                for seizure in all_seizures_global:
                    seizure_start = seizure['start_sec_global']
                    dt = seizure_start - last_segment_end_global
                    
                    if i_end_sec <= dt <= i_start_sec:
                        sequence_type = 'interictal'
                        break

        elif TASK_MODE == 'detection':
            # Basic detection logic (check for overlap)
            for seizure in all_seizures_global:
                seizure_start = seizure['start_sec_global']
                seizure_end = seizure['end_sec_global']
                if sequence_start_global < seizure_end and sequence_end_global > seizure_start:
                    sequence_type = 'ictal'
                    matched_seizure_id = seizure.get('seizure_id')
                    break
        
        # SKIP if it didn't match any class
        if sequence_type is None:
            sequence_start_local += stride_duration # <--- Now safe because stride_duration is defined at top
            continue
    ...
    if VERBOSE_WARNINGS and skipped_boundary_sequences > 0:
        print(f"  {filename}: Skipped {skipped_boundary_sequences} sequences due to insufficient boundary data")
