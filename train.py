...
class EEGDataset(Dataset):
    ...
    def __init__(self, h5_file_path, split="train"):
        ...
        print(
            f"  - Value range: [{torch.min(self.spectrograms):.4f}, {torch.max(self.spectrograms):.4f}]"
        )
...
class MetricsTracker:
    ...
    def compute_metrics(self):
        ...
        metrics = {
            ...
            "auc_roc": (
                roc_auc_score(true_np, prob_np) if len(np.unique(true_np)) > 1 else 0.0
            ),
        }
...
class EEGCNNTrainer:
    ...
    def _create_dataloader(self, split):
        ...
        dataloader = DataLoader(
            ...
            pin_memory=(
                True if self.device.type == "cuda" else False
            ),  # Only CUDA supports pin_memory
        )
    ...
    def train_epoch(self, epoch):
        ...
        pbar = tqdm(
            self.train_loader, desc=f"Training Epoch {epoch+1}/{TRAINING_EPOCHS}"
        )
    ...
    def validate_epoch(self, epoch):
        ...
        with torch.no_grad():
            pbar = tqdm(
                self.val_loader, desc=f"Validation Epoch {epoch+1}/{TRAINING_EPOCHS}"
            )
    ...
    def plot_training_curves(self):
        ...
        for idx, metric in enumerate(metrics_to_plot):
            ...
            ax.plot(
                epochs, train_values, "o-", label="Train", linewidth=2, markersize=4
            )
